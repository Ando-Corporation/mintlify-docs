...
Ando MCP includes tools for searching, listing, and fetching messages, members, conversations, clipboards, calls/Jams, transcripts, threads, and tasks. It also includes write tools for explicit user-requested actions like sending messages, replying in threads, sending DMs, reacting to messages, inviting workspace members, recording task updates, and adjusting channel proactivity. See the tool reference at the bottom of this page for the full list.
...
| `search_tasks` | Search visible tasks, or return recent visible tasks with an empty query. | Optional: `query`, `limit` (default 10, max 25). |
...
| `get_task` | Fetch task summary, recent work log entries, and active task URLs. | `task_id` required. |
| `record_task_update` | Append a task log entry, patch lightweight state, and add or update task URLs atomically. | `task_id` plus the task update body. |
| `update_channel_proactivity` | Adjust how active an eligible managed-agent member should be in the current channel. | `turn_id`, `message_id`, `level`, `reason`. Optional: `conversation_id`, `custom_instructions`. |
